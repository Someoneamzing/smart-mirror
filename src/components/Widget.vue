        ...
        <table class="widget-settings-scroll">
          ...
          <tr class='widget-setting-row' v-for="setting in Object.keys(typeInfo.settings)" :key="setting">
            <td class="setting-label-cell"><label>{{typeInfo.settings[setting].name}}</label></td><td class="setting-value-cell">
              <textarea v-if="typeInfo.settings[setting].type === 'string' && typeInfo.settings[setting].large === true" @change="$store.commit('widgets/updateSetting', {id, prop: setting, value: $event.target.value})" :value="$store.getters['widgets/setting'](id, setting)" ></textarea>
              <input v-else-if="['string','number','boolean'].includes(typeInfo.settings[setting].type)" :type="{'string':'text','number':'number','boolean':'checkbox'}[typeInfo.settings[setting].type]" :checked="$store.getters['widgets/setting'](id, setting)" :value="$store.getters['widgets/setting'](id, setting)" @change="$store.commit('widgets/updateSetting', {id, prop: setting, value: $event.target[typeInfo.settings[setting].type==='boolean'?'checked':'value']})">
